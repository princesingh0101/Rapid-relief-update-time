...
import { Eye, EyeOff, Mail, Lock, HeartPulse, Moon, Sun, Store } from 'lucide-react';
...
const StoreLogin: React.FC = () => {
  ...
  const { login, loading, isAuthenticated, isStore } = useAuth();
  ...
  // If already logged in as store, redirect to dashboard
  useEffect(() => {
    if (!loading && isAuthenticated && isStore) {
      navigate('/store/dashboard');
    }
  }, [loading, isAuthenticated, isStore, navigate]);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (email && password) {
      ...
      if (success) {
        // useAuth login should handle the session. If it's a store, go to dashboard.
        navigate('/store/dashboard');
      } else {
      ...
      <div className="w-full max-w-md">
        <div className="bg-white dark:bg-slate-900 rounded-[32px] shadow-2xl dark:shadow-none border border-slate-100 dark:border-slate-800 p-10 relative overflow-hidden">
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-slate-500 dark:text-slate-400 font-bold text-sm mt-1 uppercase tracking-widest">Store Portal</p>
          </div>

          <div className="text-center mb-8">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">Welcome Back</h2>
            <p className="text-slate-400 dark:text-slate-500 text-sm mt-1">Sign in to access the store dashboard</p>
          </div>

          <form onSubmit={handleLogin} className="space-y-5">
            <div>
              <label className="block text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 ml-1">Store Email</label>
              <div className="relative group">
                ...
                <input 
                  ...
                  placeholder="store@rapidrelief.com"
